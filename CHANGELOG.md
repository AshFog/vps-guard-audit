...
- 在创建输出目录、报告文件、运行锁和临时目录之前完成自定义配置的权限、所有者、符号链接、语法和值域校验。
- 配置被拒绝时不再显示检测开始提示，也不会残留空报告文件。
- 补充配置覆盖后的 `POLICY` 与 `MAX_LIST_ITEMS` 校验，以及对应的自动化回归测试。
- 新增受保护的真实 VM 验收助手，拒绝在容器或未明确声明的一次性主机上建立发布证据。
- 对 `HARD-2001` 至 `HARD-2008` 分别记录第二终端确认和五分钟超时回滚，共16条发布门槛。
- 确认路径要求提交后再由备用管理员打开第三条 SSH 会话；超时路径核对事务原因、systemd service 结果和至少270秒的时间证据。
- 生成不含 IP、端口、用户名、主机名或快照名称的 PR 摘要，并将原始证据保存为 `0600` 私有归档。

## 6.0.0-dev.8

- 新增覆盖全部18项动作的统一加固测试入口，并补齐 `HARD-1001` 至 `HARD-1004` 的执行、验证与事务回滚测试。
- 新增 Ubuntu 26.04/24.04/22.04 与 Debian 13/12/11 发行验收矩阵；每个系统均在无网络一次性容器中运行完整隔离动作套件。
- 新增受保护的安装生命周期测试，验证全新安装、真实 v5.0.0 至 v6 升级、状态保留、版本切换、安装权限、完整性清单和升级后只读审计。
- 破坏性安装验收只有在 root、非 SSH 会话且明确设置 `VPSGA_E2E_DISPOSABLE=YES` 时才可运行，防止误用于生产 VPS。
- 新增 v6 人工 VM 发布清单，单独验收真实 systemd、OpenSSH、UFW、Fail2ban、第二终端确认与超时恢复；容器测试不冒充真实网络验收。
