...
> v6.0.0 正在开发：在中文原生只读审计之后，增加面向 Ubuntu/Debian 常见 VPS 的 18 项可控加固。当前 `HARD-1001` 至 `HARD-1007` 开放交互确认执行，其余项目仍只生成计划。
...
当前可执行项目为 `HARD-1001` 至 `HARD-1007`：四项文件权限修复，以及禁止 SSH 空密码、限制 SSH 认证尝试次数、关闭 X11 转发。执行前必须在终端输入 `APPLY`，工具会为每个动作建立独立事务，保存原文件及元数据，并在修改后验证失败时自动回滚。root/密码登录方式、防火墙、网络和服务类连接敏感项目仍不可执行。
