...
> v6.0.0 正在开发：在中文原生只读审计之后，增加面向 Ubuntu/Debian 常见 VPS 的 18 项可控加固。当前仅四项文件权限修复开放交互确认执行，其余项目仍只生成计划。
...
第一批可执行项目为 `HARD-1001` 至 `HARD-1004`。执行前必须在终端输入 `APPLY`，工具会为每个动作建立独立事务，保存原文件及元数据，并在修改后验证失败时自动回滚。SSH 登录方式、防火墙、网络和服务类项目仍不可执行。
