...
> v6.0.0 正在开发：在中文原生只读审计之后，增加面向 Ubuntu/Debian 常见 VPS 的 18 项可控加固。当前 10 项常规安全加固已开放交互确认执行；8 项连接敏感加固仍只生成计划，防失联前置检查与第二终端验证框架已经建立。
...
当前可执行项目为 `HARD-1001` 至 `HARD-1010`：四项文件权限修复、三项低风险 SSH 设置、自动安全更新、兼容性较高的 sysctl 基线和 Core Dump 限制。执行前必须在终端输入 `APPLY`，工具会为每个动作建立独立事务，保存原文件及元数据，并在修改后验证失败时自动回滚。`HARD-1008` 新安装的软件包不会因配置回滚而被自动卸载，避免破坏软件包依赖；其 VPSGA 配置仍会恢复。root/密码登录方式、防火墙、网络和服务类连接敏感项目仍不可执行。

在未来开放连接敏感动作前，可以先运行 `vpsga connection-check`。它会检查当前 SSH 入口、具备安全公钥的非 root sudo 管理员，以及 systemd 延时回滚支持；VPS 控制台是否真正可用仍需用户人工确认。
